...
public class OutpassRequestAdapter extends ArrayAdapter<OutpassRequest> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // Disable buttons based on the request status
        boolean isProcessed = "accepted".equals(request.getStatus()) || "rejected".equals(request.getStatus());
        approveButton.setEnabled(!isProcessed);
        rejectButton.setEnabled(!isProcessed);

        // Set click listeners for buttons
        approveButton.setOnClickListener(v -> {
            callback.updateRequestStatus(request.getRequestId(), "accepted");
        });

        rejectButton.setOnClickListener(v -> {
            callback.updateRequestStatus(request.getRequestId(), "rejected");
        });
